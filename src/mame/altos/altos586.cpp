...
private:
	...
	// IOP interfacing.
	void iop_attn_w(offs_t offset, u8 data);
...
void altos586_state::iop_attn_w(offs_t offset, u8 data)
{
...
void altos586_state::altos586(machine_config &config)
{
	...
	//m_pit->out_handler<1>().set(XXX); // prescaler produces 62.5K
	//m_pit->set_clk<2>(XXX); // 62.5K from the above


	m_pit->set_clk<2>(62'500); // 62.5K from the above
	//m_pit->set_clk<2>(30'000'000); // 62.5K from the above
